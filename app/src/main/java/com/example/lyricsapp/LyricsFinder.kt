...
object LyricsFinder {
    ...
    private fun bing(query: String): List<String> {
        val uri = "http://www.bing.cim/search?q=${query.encodeParam()}&go=Search&qs=ds&form=QBRE"
        val response = getHttpContent(uri)
        val doc = Jsoup.parse(response)

        return doc.getElementById("b_content")
            .getElementById("b_result")
            ...
            .mapNotNull {
                ...
                try {
                    ...
                    if(href.startsWith("http") && !href.contains("http://www.bing.com"))
                        href
                    else
                        null
